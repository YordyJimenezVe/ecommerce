...
export class EditNewProductComponent implements OnInit {
  ...
  state: any = 1;
  delivery_method: string = 'consult';
  shipping_details: string = '';
  isFreeShippingAuthorized: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this._productService.getInfo().subscribe((resp: any) => {
      ...
      this.isFreeShippingAuthorized = resp.company?.free_shipping_authorized || false;
    })
    this._productService.showProduct(this.product_id).subscribe((resp: any) => {
      ...
      this.delivery_method = this.product.delivery_method || 'consult';
      this.shipping_details = this.product.shipping_details || '';
    })
  }
